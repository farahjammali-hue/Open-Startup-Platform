...
**Implemented (Phase 3a):** `server/canonical.ts` resolves clusters 1–4 and cluster 9's HQ into `{value, source, asOf}` facts; readers go through `storage.canonicalFactsFor` — the Claude connector's `get_startup_profile` and `startup_metric_summary`, and the investment-application snapshot. **Phase 3b:** `rev_monthly_change` and `sales_ltv_cac` carry `derived: true` in `shared/metricsCatalog.ts` and are recomputed from their inputs at display time (charts). **Phase 4 (merge):** the write side is unified too — the per-cluster notes below say what changed.

1. **Team size / headcount** — CANONICAL: latest `hr_team_size` (+ other `hr_*`) metric → Card 4 `startups.team_size` → `count(team_members)`. `team_members` is in practice the founders list. Known mislabels: `youth_employees` (count stored, % shown), `hr_pct_youth` (a real %). **Merged (4.2):** the Card-4 inputs are retired — the UI shows read-only mirrors of the latest `hr_*` metrics (legacy columns only show through for startups that never reported), and the youth stat now displays `hr_pct_youth`, the real %.
2. **Funding raised** — CANONICAL: Card 6 `total_funding_raised` → `sum(startup_funding_rounds.amount)` → survey `amount_raised` (legacy). Grants: `total_funding_non_dilutive` → latest `fund_grants` → `total_grants` (legacy). Valuation: latest `fund_valuation` → `last_valuation` (legacy). Round terms/size: funding_rounds rows are the detail; `round_terms`/`total_round_size` legacy. **Merged (4.1):** every startup with legacy money figures got an "initial"-period metric baseline backfilled by Schema Batch 4 (founder-entered metric values always win); the legacy columns are resolver fallbacks only.
...
4. **Stage & program track** — `kys_profiles.track` is THE program cohort (drives training visibility, messaging, filters); `startups.stage` is the maturity scale; `product_stage`/`trl_level` are product-specific scales; `investment_stage` duplicated the track and drove nothing — input retired in 4.3; Card 6 now shows the startup's actual KYS track, read-only. Alumni-ness: `users.role='alumni'` (access) + `startups.graduated_at` (fact/date).
5. **Clients & partners** — CANONICAL: the CRM (`startup_crm_entries`) for relationships; monthly `sales_*`/`partner_*` metrics for counts over time. Card 13/14 stats are static copies, and the 6 partner types are spelled 3 ways across catalogs — the one cluster still open after Phase 4 (backlog: derive the cards from CRM + metrics, unify the type vocabulary).
...
8. **Session recaps** — CANONICAL: the AI recap fields (same 6 names on both notes tables; on training, `mentor_comments` holds the trainer's comments). Legacy `points_discussed`/`what_is_going_well`/`what_is_not_going_well`(+`action_items`) are superseded on mentorship and retired from training writes in 4.7 — the caller-less founder recap endpoint was removed; old rows still display read-only. One quirk: a cohort training recap is copied into every visible startup's row by design.
9. **Geography & incorporation** — CANONICAL HQ: `location`. Card 1 `country` secondary; `country_of_incorporation`, `users.country`, KYS address fields, Section VI `legal_domicile` are ghosts/dormant — `country_of_incorporation`, `customer_base` and `sdgs_addressed` left the zod round-trips in 4.4/4.5 (columns stay, read-never). Incorporation: `legal_entity_status` canonical; `is_incorporated`, KYS `incorporated`, Section VI `legal_incorporated` secondary/dormant.
10. **Data room** — CANONICAL: `data_room_link` (+ `documents` rows where uploads are used). Checklist: `metricsCatalog.DATA_ROOM_ITEMS` is the ONE catalog since 4.8 — the unused client `dataRoomChecklist.ts` was deleted, old `checklist_key` slugs were remapped by Schema Batch 4 and unknown ones detached. The document uploads and the expiring investor share links got their founder UI in 4.9 (Data Room → Documents / Share with investors). Individual link columns (deck, roadmap, GTM, CRM links) intentionally stay per-card.
...
**Dead code paths — resolved in Phase 4.9 (Ghazi's calls, 2026-09-26):** founder School routes DELETED (the admin /admin/school CRUD stays); KYS-document upload route DELETED (existing rows still listed read-only); founder document + share-link routes REVIVED with a real UI (Data Room → Documents / Share with investors tabs); `dataRoomChecklist.ts` and `BUSINESS_MODEL_LABELS`/`CUSTOMER_BASE_LABELS` DELETED. The old `kpi_submissions` history was exported to CSV for safekeeping (table untouched).
