...
function SessionDetailModal({ session: s, onClose }: { session: TrainingSession; onClose: () => void }) {
  return (
    ...
    >
      ...
      {/* 2d: the AI session recap was saved to the database but never shown
          to founders. Same layout as the mentorship recap. */}
      {(s.notes.aiGeneratedAt || s.notes.progressHighlights) && (
        <div className="mt-5 border-t border-slate-100 pt-4">
          <div className="mb-2 flex items-center justify-between">
            <p className="ost-label !mb-0">Session recap</p>
            {s.notes.aiGeneratedAt && (
              <span className="text-xs text-slate-400">Generated by AI · {new Date(s.notes.aiGeneratedAt).toLocaleDateString()}</span>
            )}
          </div>
          <div className="space-y-3">
            {([
              ["Team members presence", s.notes.teamMembersPresence],
              ["Progress / highlights", s.notes.progressHighlights],
              ["Trainer comments", s.notes.mentorComments],
              ["Needs highlighted", s.notes.needsHighlighted],
              ["To check next meeting", s.notes.nextMeetingCheckIns],
              ["Action items for Open Startup", s.notes.actionItemsForOst],
