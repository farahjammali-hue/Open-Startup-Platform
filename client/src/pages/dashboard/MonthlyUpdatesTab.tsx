...
import { MONTHLY_STATUS_LABELS, MONTHLY_STATUS_TONES, type MonthlyUpdateRow } from "./types";
...
export function MonthlyUpdatesTab({ updates }: { updates: MonthlyUpdateRow[] }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      showToast("Quarterly update submitted.");
    ...
    <div className="space-y-6">
      <form onSubmit={submit} className="ost-card p-8">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="ost-card-title">Quarterly check-in</h2>
            <p className="mt-1 ost-helper-text">Brief check-in due by the 5th of each quarter</p>
          </div>
        ...
        <div className="space-y-4">
          <div>
            <label className="ost-label">What was achieved this quarter? <span className="text-secondary">*</span></label>
          ...
          <div>
            <label className="ost-label">Focus for next quarter <span className="text-secondary">*</span></label>
      ...
      {updates.length === 0 ? (
        <EmptyState
          icon={Target}
          title="No quarterly updates yet"
          description="Your first check-in above will show up here, and every quarter after that."
        />
      ) : (
            ...
            {updates.map((u) => (
              <div key={u.id} className="rounded-lg border border-slate-100 p-4">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-semibold text-primary">Q{u.periodQuarter} {u.periodYear}</p>
